...
class MatchProgressEntity(
    ...
    fun toDomain() =
        MatchProgress(
            ...
            redFinishedAt = redFinishedAt?.let { Instant.ofEpochMilli(it) },
            blueFinishedAt = blueFinishedAt?.let { Instant.ofEpochMilli(it) },
            timerStartedAt = timerStartedAt?.let { Instant.ofEpochMilli(it) },
    ...
    companion object {
        fun MatchProgress.fromDomain(
            ...
            redFinishedAt = this.redFinishedAt?.toEpochMilli(),
            blueFinishedAt = this.blueFinishedAt?.toEpochMilli(),
            timerStartedAt = this.timerStartedAt?.toEpochMilli(),
